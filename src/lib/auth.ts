...
export type SessionPayload = BarberSession | PlatformSession;
...
export async function verifySessionToken(
  ...
  try {
    ...
    if (payload.kind === "barber") {
      if (
        typeof payload.barberId !== "string" ||
...
export async function requireBarberSession(slug?: string) {
  const session = await getSession();
  if (!session || session.kind !== "barber") return null;
